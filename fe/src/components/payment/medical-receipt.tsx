...
export const MedicalReceipt = forwardRef<HTMLDivElement, MedicalReceiptProps>(({ bookingData }, ref) => {
    ...
    useEffect(() => {
        const generateQR = async () => {
            if (
                ...
                try {
                    const qrCode = await QRCodeGenerator.generateAppointmentQR({
    ...
    return (
        <div ref={ref} className="bg-white">
            <Card className="max-w-2xl mx-auto shadow-lg border-2 border-teal-100">
                        ...
                        {qrCodeDataURL && (
                            <div className="text-center mb-8 p-4 bg-gray-50 rounded-lg">
                                <Image
                                    ...
                                    height={150}
                                />
                                <p className="text-sm text-gray-600 font-medium">Mã QR xác minh lịch khám</p>
                                <p className="text-xs text-gray-500">Xuất trình mã này khi đến khám</p>
                            </div>
                        )}
